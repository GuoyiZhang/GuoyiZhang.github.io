...
本文不是对原文进行完整的翻译，主要是作者消化后整理出来的文档，需要看原文的请直接访问原文地址[https://ethereum.org/token](https://ethereum.org/token)。

挖矿：[https://ethereum.org/token/#proof-of-work](https://ethereum.org/token#proof-of-work)
...
**实际的交易系统需要考虑这个问题。至少要有提示。目前的DAO出现了这个问题，DAO中的以太币已经被白猫黑客转移到其他地方。现在如果通过分裂的方式兑换DAO币，会导致得不到以太币。造成资金损失。具体内容请参考[http://ethfans.org/topics/404](http://ethfans.org/topics/404)。**
